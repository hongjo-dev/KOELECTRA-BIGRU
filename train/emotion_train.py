...
try:
    for epoch in range(epochs):
        ...
        torch.save(model, f'/pth/Emotion{epoch+1}.pth')
    ...
    # 라벨 그룹화 및 분류 보고서 출력
    label_map = {0: 'O', 1: 'B-E', 2: 'I-E'}  # 라벨 맵 정의
    ...
    report2 = classification_report(grouped_true_labels, grouped_pred_labels, target_names=['Non-Emotion', 'Emotion'])
    report = classification_report(grouped_true_labels, grouped_pred_labels, target_names=['Non-Emotion', 'Emotion'],output_dict=True)
    print(report2)
    Emotion_f1_score = report['Emotion']['f1-score']
    print(f"Emotion 클래스 F1 스코어: {Emotion_f1_score:.4f}")
