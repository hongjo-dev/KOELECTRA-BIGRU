...
model_path = '/pth/Attribute3.pth' 
model = torch.load(model_path)
model.eval()


def group_attribute_labels(labels, label_map):
    return [1 if label_map.get(label, 'O') in ['B-A', 'I-A'] else 0 for label in labels]

all_preds = []  
all_labels = [] 

# 로그 파일 생성
log_file = '/log/attribute_model_output_log.txt'
...
with torch.no_grad(), open(log_file, 'w', encoding='utf-8') as log:
    ...
    label_map = {0: 'O', 1: 'B-A', 2: 'I-A'}
    grouped_true_labels = group_attribute_labels(true_labels, label_map)
    grouped_pred_labels = group_attribute_labels(pred_labels, label_map)
    report2 = classification_report(grouped_true_labels, grouped_pred_labels, target_names=['Non-attribute', 'attribute'])
    report = classification_report(grouped_true_labels, grouped_pred_labels, target_names=['Non-attribute', 'attribute'], output_dict=True)
    attribute_f1_score = report['attribute']['f1-score']
    log.write(f"성능 평가 결과:\n{classification_report(grouped_true_labels, grouped_pred_labels, target_names=['Non-attribute', 'attribute'])}\n")
    log.write(f"attribute 클래스 F1 스코어: {attribute_f1_score:.4f}\n")
    log.write(f"모델 종료 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
    print(report2)
    print(f"Attribute 클래스 F1 스코어: {attribute_f1_score:.4f}")
